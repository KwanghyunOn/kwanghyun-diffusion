...
def sample_ddpm(x, model, betas):
    ...
    # noise_schedule = NoiseScheduleVP('discrete', alphas_cumprod=torch.from_numpy(diffusion.alphas_cumprod))
    ...
    #     out = model_fn(x, t, **model_kwargs)
    #     if model_config.learn_sigma:
...
def ddpm_steps(x, timesteps, betas):
    with torch.no_grad():
        n = x.size(0)
        timesteps_next = [-1] + list(timesteps[:-1])
        xs = [x]
        x0_preds = []
        for i, j in zip(reversed(timesteps), reversed(timesteps_next)):
